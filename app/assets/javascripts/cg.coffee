class CheckGraph
	urls = 
		addChild : (parentId, childId) ->
			return "/link_task?graph_id=" + graphId + "&task_parent=" + parentId + "&task_child=" + childId;
		changeStatus : (taskId, status) ->
			return "/change_status?task_id=" + taskId + "&status=" + status
		createTask : (name) ->
			return ("create_new_orphan_child?graph_id=" + graphId + "&name=") + encodeURIComponent(name)
	graphId = null
	name = null
	tasks = {}
	tasksCount = 0
	...
	handleGraphics = (cleanup) ->
		...
		findHeight = () ->
			maxTasksInLevel = 0
			markMax = (count) ->
				if count > maxTasksInLevel
					maxTasksInLevel = count
			markMax level.length for level in levels
			return yOffSet + maxTasksInLevel * (height + ySpace)
		canvasHeight = findHeight()
		paper = Raphael("background-canvas", "#{canvasWidth}px", "#{canvasHeight}px")
