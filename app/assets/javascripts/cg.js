...
CheckGraph = (function() {
  var addConnectionOnServer, addFloatingPlus, addNewTask, graphId, handleClass, handleGraphics, initGraph, initTasksObj, levels, markTaskAs, name, populateLevels, tasks, tasksCleanup, tasksCount, urls;
  ...
  handleGraphics = function(cleanup) {
    var addChildConnection, canvasHeight, canvasWidth, connectionClickHandle, connectionClickStatus, connectionDrawer, dataContainer, drawLevel, findHeight, height, id, level, paper, task, width, xOffSet, xSpace, yOffSet, ySpace, yTextOffSet, _i, _len, _results;
    ...
    yTextOffSet = 30;
    findHeight = function() {
      var level, markMax, maxTasksInLevel, _i, _len;
      maxTasksInLevel = 0;
      markMax = function(count) {
        if (count > maxTasksInLevel) {
          return maxTasksInLevel = count;
        }
      };
      for (_i = 0, _len = levels.length; _i < _len; _i++) {
        level = levels[_i];
        markMax(level.length);
      }
